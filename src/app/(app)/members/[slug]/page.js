...
import { ArrowLeft, MessageCircle, Link as LinkIcon, MapPin, Pencil, Trash2, Mail } from "lucide-react";
import Button from "@/app/components/ui/Button";
import { getMember, getMyProfile, deleteAccount } from "@/app/lib/profileService";
...
export default function MemberProfilePage() {
  ...
  function goBackToMembers() {
    if (fromAdmin) { router.push("/admin/users"); return; }
    if (fromOnboarding) { router.push("/"); return; }  // no useful history after onboarding
  ...
  useEffect(() => {
    ...
    if (!num) { setMember(null); return; }
    getMember(num).then(setMember).catch(() => setMember(null));
    ...
    return (
      <>
        <div className="py-16 text-center">
          ...
          <button onClick={() => router.back()} className="mt-3 text-sm text-brand-blue">Go back</button>
        </div>
      </>
    );
  }
  ...
  return (
    <>
      <div className="mb-4 flex items-center justify-between">
        <button onClick={goBackToMembers} className="flex items-center gap-1.5 text-xs text-slate-500 hover:text-brand-navy">
          <ArrowLeft size={14} /> {fromAdmin ? "Back to members list" : "Back to members"}
        </button>
        {isMe && (
          <button
      ...
      <div className="grid grid-cols-1 gap-5 md:grid-cols-3">
        ...
        <div className="md:col-span-1">
          <div className="rounded-2xl border border-slate-200 bg-white p-6 text-center shadow-sm">
            ...
            <h1 className="text-lg font-semibold text-brand-navy">{member.full_name}</h1>
            ...
            {member.primary_link && (
              <a href={ensureUrl(member.primary_link)} target="_blank" rel="noreferrer"
            ...
            {member.links?.linkedin && (
              <a href={ensureUrl(member.links.linkedin)} target="_blank" rel="noreferrer"
            ...
            {waLink && (
              <a href={waLink} target="_blank" rel="noreferrer" className="mt-3 block">
                <Button>
                  <span className="flex items-center gap-2"><MessageCircle size={16} /> Contact on WhatsApp</span>
                </Button>
              </a>
            )}

            {!waLink && !member.email && !member.primary_link && !member.links?.linkedin && (
              <p className="mt-4 text-xs text-slate-400">This member hasn&apos;t shared a contact channel.</p>
            )}

            <p className="mt-4 text-[11px] text-slate-400">Kuzana member #{member.member_number}</p>
          </div>
        </div>
        ...
        <div className="space-y-5 md:col-span-2">
          {member.intro && (
            <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
              <p className="text-sm leading-relaxed text-slate-600">{member.intro}</p>
            </div>
          )}
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            {member.industries?.length > 0 && (
              <div className="mb-5">
                <div className="mb-2 text-xs font-semibold text-slate-500">Industry</div>
                <div className="flex flex-wrap gap-2">
                  {member.industries.map((i) => (
                    <span key={i.id} className="rounded-full bg-slate-100 px-3 py-1 text-xs text-slate-600">{i.label}</span>
                  ))}
                </div>
              </div>
            )}
            <Section title="Offers" items={member.offerings} tone="offer" />
            <Section title="Looking for" items={member.looking_for} tone="need" />
          </div>
        </div>
      </div>
...
function Section({ title, items, tone }) {
  ...
  const cls = tone === "offer" ? "bg-brand-blue-50 text-brand-blue-700" : "bg-slate-100 text-slate-600";
  return (
    <div className="mb-5 last:mb-0">
      <div className="mb-2 text-xs font-semibold text-slate-500">{title}</div>
      <div className="flex flex-wrap gap-2">
        {items.map((it, i) => (<span key={i} className={"rounded-full px-3 py-1 text-xs " + cls}>{it}</span>))}
      </div>
    </div>
  );
}
...
function prettyLink(url = "") { return url.replace(/^https?:\/\//, "").replace(/\/$/, ""); }
