...
export default function VerifyPage() {
  ...
  async function handleResend() {
    ...
    setError("");
    try {
      await sendOtp(pending.whatsapp_number, pending.channel || "whatsapp");
  ...
  // Send to the OTHER channel (fallback: WhatsApp <-> email).
  async function handleSwitchChannel() {
    if (!pending) return;
    const other =
      (pending.channel || "whatsapp") === "email" ? "whatsapp" : "email";
    setError("");
    try {
      await sendOtp(pending.whatsapp_number, other);
      const updated = { ...pending, channel: other };
      setPendingState(updated);
      notify(
        other === "email"
          ? "Code sent to your email instead."
          : "Code sent to your WhatsApp instead.",
        "success",
        3000,
      );
      setCooldown(RESEND_SECONDS);
      ...
      setError(
        err.response?.data?.detail || "Couldn't switch. Please try again.",
      );
    }
  }
  ...
  const onEmail = (pending?.channel || "whatsapp") === "email";
  ...
  return (
    ...
    >
      {/* Channel-aware cue: WhatsApp or email, whichever the code was sent to. */}
      {onEmail ? (
        <div className="mb-4 flex items-center justify-center gap-2 rounded-lg bg-brand-blue-50 px-3 py-2.5 text-sm font-medium text-brand-blue-700">
          <span>Code sent by email to {pending.email || "your email"}</span>
        </div>
      ) : (
          ...
          <div className="mt-1">
            ...
            >
              {onEmail
                ...
                : "Can't get it? Send to email instead"}
            </button>
          </div>
        </div>
      </form>
